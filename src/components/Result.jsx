...
const Result = () => {
  ...
  useEffect(() => {
    ...
    resultData.forEach(data => {
      ...
      else if (data.answerSelected === data.answer[0]) {
        right++;
      } else {
        wrong++;
      }
  ...
  return (
        <div>
          <div className='flex justify-center gap-5 mb-5'>
            <div className='badge badge-success'>Correct: {totalRight}</div>
            <div className='badge badge-error'>Wrong: {totalWrong}</div>
            <div className='badge badge-warning'>Unanswered: {totalUnanswered}</div>
          </div>
          <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 justify-center'>
              {resultData.map((data, index) => { 
                ...
                const notSelected = data.answerSelected === '';
                const isAnswerCorrect = data.answerSelected === rightAnswer;
                const borderColor = notSelected ? 'border-warning' : noRightOption ? 'border-warning' : isAnswerCorrect ? 'border-success' : 'border-error';
                  ...
                  <div key={index} className={`card bg-base-100 w-96 shadow-xl border-2 ${borderColor}`}>
                    {
                      ...
                      :
                      notSelected ? 
                      <div className='badge badge-warning mx-auto mt-3'>Unanswered</div>
                      :
                      isAnswerCorrect ?
